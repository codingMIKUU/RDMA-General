...
void test_xrc_ini(struct ibv_pd* pd, union ibv_gid* gid, struct ibv_xrcd* xrcd,
  ...
  memset(buf, 1, reg_size);
  // memset(buf,-1,100);
  struct ibv_mr* mr =
      ibv_reg_mr(pd, buf, reg_size,
                 IBV_ACCESS_LOCAL_WRITE | IBV_ACCESS_REMOTE_WRITE |
                     IBV_ACCESS_REMOTE_READ | IBV_ACCESS_REMOTE_ATOMIC);
  // char *buf2 = (char*)malloc(reg_size);
  // memset(buf2,0,reg_size);

  // struct ibv_mr *mr2 = ibv_reg_mr(pd,buf2,reg_size,IBV_ACCESS_LOCAL_WRITE |
...
#define TCP_PORT 10086
  ...
  if ((newsockfd =
           accept(sockfd, (struct sockaddr*)&client_addr, &client_len)) < 0) {
  ...
  wr.wr.rdma.remote_addr = remote_qp_info.addr + 2;
