...
void print_qp_info(struct qp_info* info) {
  printf("QP Number: %u\n", info->qpn);
  printf("GID: ");
  for (int i = 0; i < 16; ++i) {
...
void recv_device_fd(int* cmd_fd) {
    ...
    struct sockaddr_un addr = {
        .sun_family = AF_UNIX,
        .sun_path = "/tmp/fd-pass.sock"
    };
    if(connect(sockfd,(struct sockaddr*)&addr,sizeof(addr))<0){
        perror("connect");
        return;
    }
    *cmd_fd = recv_fd(sockfd);

    close(sockfd);
}
int get_rdma_attrs(struct ibv_context** ibv_ctx, uint8_t* port_id,
                   union ibv_gid* gid) {
  int num_devices = 0;
  struct ibv_device** dev_list = ibv_get_device_list(&num_devices);
  if (dev_list == NULL) {
    printf("Failed to get InfiniBand device list,error:%s\n", strerror(errno));
    return -1;
  }
  ...
  for (int dev_i = 1; dev_i < num_devices; dev_i++) {
    ...
    // int cmd_fd;
    // recv_device_fd(&cmd_fd);
    // struct ibv_context* ib_ctx = ibv_import_device(cmd_fd);
    if (ib_ctx == NULL) {
      printf("Failed to open dev,error:%s\n", strerror(errno));
      return -1;
    }
...
void test_xrc_ini(struct ibv_pd* pd, union ibv_gid* gid, struct ibv_xrcd* xrcd,
  ...
  // struct ibv_ah *ah =
  // ibv_create_ah(pd,ah_attr,xrcd,&local_qp_info,&remote_qp_info); struct
  // ibv_qp *tgt_qp = create_xrc_tgt(pd,xrcd,&local_qp_info,&remote_qp_info);
  ...
  conn_attr.qp_access_flags = IBV_ACCESS_REMOTE_WRITE | IBV_ACCESS_REMOTE_READ |
...
#define TCP_PORT 10086
  ...
  // 发送本地 RDMA 信息
  if (send(sockfd, &local_qp_info, sizeof(struct qp_info), 0) !=
      sizeof(struct qp_info)) {
    perror("Failed to send local RDMA info");
    exit(EXIT_FAILURE);
  }
  // 接收远端 RDMA 信息
  if (recv(sockfd, &remote_qp_info, sizeof(struct qp_info), 0) !=
      sizeof(struct qp_info)) {
